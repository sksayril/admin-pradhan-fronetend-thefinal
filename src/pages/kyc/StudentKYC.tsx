...
import { GraduationCap, CheckCircle, XCircle, Clock, Eye, FileImage, AlertCircle, RefreshCw, Filter, Search } from 'lucide-react';
import { kycService, StudentKYC } from '../../services';
import DocumentPreviewModal from '../../components/DocumentPreviewModal';

const StudentKYCPage: React.FC = () => {
  const [studentKYC, setStudentKYC] = useState<StudentKYC[]>([]);
  ...
  const loadStudentKYC = async () => {
    setLoading(true);
    setError(null);
    
    try {
      const data = await kycService.getStudentKYCByStatus(selectedStatus);
      setStudentKYC(data);
    } catch (err) {
      console.error('Error loading student KYC:', err);
      setError('Failed to load student KYC requests. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const handleApprove = async (kyc: StudentKYC) => {
    setSelectedKYC(kyc);
    setShowActionModal('approve');
    setRemarks('');
  };

  const handleReject = async (kyc: StudentKYC) => {
    setSelectedKYC(kyc);
  ...
  const filteredKYC = studentKYC.filter(kyc =>
    kyc.studentId.firstName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    kyc.studentId.lastName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    kyc.studentId.studentId.toLowerCase().includes(searchTerm.toLowerCase()) ||
    kyc.studentId.email.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Student KYC Management</h1>
        </div>
        <button
          onClick={loadStudentKYC}
          disabled={loading}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-800">
            Student KYC Requests ({filteredKYC.length})
          </h2>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="p-6">
              ...
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-500">{selectedKYC.studentId.studentId}</p>
              </div>
